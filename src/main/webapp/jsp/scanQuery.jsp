...
<html>
	...
	<script>
		...
		if( '<%=session.getAttribute("tabAuth")%>' == '1' )
			allCarriers = ['BFHY','BFLTA','CDJQ','GZLH','JJXD','JSYZ','SHAZX', 'SHSS', 'SZFLS', 'THWL', 'XKWL','ZYKY'];
		else
		...
		tabTaskInfo.on("click",".count",function(){
			...
			window.open(basePath + "taskItem?laneE=" + laneE + "&arcName=" + arcName
					+ "&cargoType=" + cargoType
					+ "&sortCode=" + sortCode
					+ "&count=" + count
		...
		function showTaskInfo(data){
			...
			for(var i = 0; i <  data.length; i++){
				htmlStr = htmlStr +
						...
						"<td class='creDate'>"+  data[i][5] +"</td></tr>";
